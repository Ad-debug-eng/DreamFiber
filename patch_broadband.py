...
# Let's perform the replacement using regular expression finding `<style>\s*\.krishi-cards-grid`
# up to `\s*<!--\s*IPTV\s*Section\s*-->`
pattern = re.compile(r'<style>\s*\.krishi-cards-grid.*?(?=\s*<!--\s*IPTV\s*Section\s*-->)', re.DOTALL)
if pattern.search(content):
    content = pattern.sub(full_broadband_section + "\n            ", content)
    with open("services.html", "w", encoding="utf-8") as f:
        f.write(content)
    print("Successfully patched broadband cards and style in services.html!")
else:
    print("Error: Could not locate broadband cards section!")
