...
<div class="krishi-cards-grid">
    {make_card("Mini", "50 Mbps", "Starter plan with OTT bundles", "Smart TV or multi-device homes should choose a higher speed.", mini_main, mini_drop, 21)}
    {make_card("Basic", "100 Mbps", "Standard streaming & OTT bundles", "Ideal for small families, smooth HD streaming on multiple devices.", basic_std_main, basic_std_drop, 24, best_seller=True)}
    {make_card("Standard", "200 Mbps", "High-speed family entertainment & OTT", "Recommended for families who want smooth multi-screen streaming without buffering.", basic_std_main, basic_std_drop, 24)}
    {make_card("Premium", "300 Mbps", "Premium streaming, gaming & OTT", "Ideal for Smart TV, WFH, online classes and uninterrupted OTT on multiple screens.", premium_main, premium_drop, 25, is_premium=True)}
</div>
"""
...
try:
    ...
    start_marker = '<style>\n        /* Page Header */'
    end_marker = '<!-- Office CTA -->'
    
    # Actually, let's just replace from <style> to <div class="krishi-cards-grid">...</div>
    # The previous code had <style> inline in broadband-ott.html before <div id="section-broadband-ott">
    
    # We can match from <style> to the end of the krishi-cards-grid
    pattern = re.compile(r'<style>.*?</style>\s*<div id="section-broadband-ott"[^>]*>\s*<div class="krishi-cards-grid">.*?</div>', re.DOTALL)
    
    # In broadband-ott.html, the style starts earlier: <style>\n        /* Page Header */...
    # Wait, the user's style in broadband-ott.html contains Page Header, Tab Toggle, Duration toggle, etc.
    # The card_style in update_services7.py only has krishi-cards-grid styles.
    # It's better to just replace <div class="krishi-cards-grid">...</div> in broadband-ott.html
    
    pattern2 = re.compile(r'<div class="krishi-cards-grid">.*?</div>\s*(?=</div>\s*</section>)', re.DOTALL)
    
    # But wait, we need to inject the CSS for k-plan-speed from card_style.
    # Let's replace the whole <div class="krishi-cards-grid"> to the end with our cards_html
    # We will inject card_style right before the grid.
    
    new_content = pattern2.sub(cards_html, content)
    
    with open("broadband-ott.html", "w", encoding="utf-8") as f:
        f.write(new_content)
except Exception as e:
    print(f"Error updating broadband-ott.html: {e}")
